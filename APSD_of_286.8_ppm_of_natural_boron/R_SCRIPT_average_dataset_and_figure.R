...
# Grouping of the files with the detector neutron counting
file_numbers <- c(1:12)
for (i in file_numbers){
  count_name <- paste('Raw_data/counts', i, sep = '')
  c_col <- read.csv(file = count_name,
                    header = FALSE,
                    sep = '\t')
  if (i == 1) {
    count_data <- c_col %>% select(V2)
    names(count_data) <- 'counts1'
  } else {
    c_col <- c_col %>% select(V2)
    names(c_col) <- paste('counts', i, sep = '')
    count_data <- cbind(count_data, c_col)
  }
}

count_means <- round(apply(counts_data, 2, mean))
hist(count_means, nclass = 12)
count_sd <- round(apply(counts_data, 2, sd))
hist(count_sd, nclass = 12)

# Grouping of the files with the spectra data
for (i in file_numbers){
...
#           file = 'R_average_APSD_286ppm_nat_boron.csv', 
#           row.names = FALSE, 
#           quote = FALSE)
...
png(filename = 'R_average_APSD_286ppm_nat_boron.png',
